...
$(document).ready(function() {
  ...
  var isHomeCheck = $('body').hasClass('is-home'),
      isBlogCheck = $('body').hasClass('is-blog');
    ...
    function renderingCircle() {
      ...
      ImgLink.forEach(function(e, i) {
        ...
        var timeoutId = setTimeout(function() {
          var circlesLength = drawedElementsInfo.length;
          drawcircle(element);
          count++;
          function deleteCircle(eIndex) {
            clearTimeout(eIndex.timeoutId);
            drawedElementsInfo[eIndex].element.clear();
            drawedElementsInfo[eIndex].element.remove();
          };
          if (count > 6 && count < circlesLength) {
            var elIndex = count - 7;
            deleteCircle(elIndex);
          } else if (count == circlesLength) {
            var elIndex = count - 7 ;
            deleteCircle(elIndex);
            var i=1;
            var lastItems = setInterval(function() {
              if (i < 7) {
                var elIndex = circlesLength + i - 7 ;
                deleteCircle(elIndex);
                i++;
              } else {
                renderingCircle();
                clearInterval(lastItems);
              };
            }, 2500);
  ...
  if (isBlogCheck) {
    $('.menu__blog').addClass('menu__link--active');
  };

  //home-blog
  function homeBlogSize() {
    var homeBlog = $('.js-blog-item'),
        homeBlogTitle = $('.js-blog-title'),
        homeBlogDesc = $('.js-blog-desc'),
        homeBlogLink = $('.js-blog-link');

    homeBlog.each(function(i) {
      let homeBlogHeight = $(this).height(),
          homeBlogTitleHeight = $(homeBlogTitle[i]).outerHeight(),
          homeBlogLinkHeight = $(homeBlogLink[i]).outerHeight(),
          homeBlogDescHeight = homeBlogHeight - homeBlogTitleHeight - homeBlogLinkHeight - 106;
      $(homeBlogDesc[i]).css({'height' : homeBlogDescHeight});
    });
  };
