...
$(function(){var i=$(".js-hiw-slider"),s=$(".js-hiw-right"),e=$(".js-hiw-left");i.slick({lazyLoad:"ondemand",slidesToShow:1,nextArrow:s,prevArrow:e,mobileFirst:!0,responsive:[{breakpoint:767,settings:{slidesToShow:2,variableWidth:!0}}]})});

var map,lat=$("#conf-map").data("coordLat"),lng=$("#conf-map").data("coordLng"),latToShow=lat+3e-4,lngToShow=lng-.0019;function initMap(){var a={lat:lat,lng:lng};map=new google.maps.Map(document.getElementById("conf-map"),{center:{lat:lat+3e-4,lng:lng-.0019},scrollwheel:!1,disableDefaultUI:!0,zoom:17,styles:[{stylers:[{saturation:-100}]}]});new google.maps.Marker({position:a,map:map,icon:"img/marker.png"})}$(window).on("load resize",function(){window.matchMedia(sm).matches&&(latToShow=lat,lngToShow=lng)});
